...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if ($buscar->rowCount() == 1){
        // Si existe
        try{
            $eliminar = $con->prepare("UPDATE proyecto SET orden = -1  WHERE orden = '$orden_anterior'");
            $eliminar->execute();
            $eliminar = $con->prepare("UPDATE proyecto SET orden = '$orden_anterior'  WHERE orden = '$orden'");
            $eliminar->execute();
